...
export default function LoginForm() {
    ...
    async function ValidarEnvioLogin() {
    ...
    return(
        <div className="w-full h-full flex flex-col gap-4">
            <button className="hover:bg-google-button-hover hover:opacity-80 hover:cursor-pointer flex items-center justify-center border border-black w-4/5 max-w-4/5 h-13 max-h-20 rounded-md mx-auto px-4 py-2">
                ...
                <span className="font-lato-bold text-md text-black">Fazer Login com o Google</span>{/*não implementado devido à falta de tempo */}
            </button> 
            ...
            <form className="flex flex-col w-full gap-14 my-4">
                ...
                <div className="relative">
                    ...
                    <button type="button" onClick={() => TrocarVisSenha()} className="absolute right-2 top-1/2 -translate-y-[95%]">
                        <img id="PasswordIcon" src="src\assets\eye-password-show.svg" alt="icone senha" className="w-6 h-6 opacity-75 mb-1"></img>
                    </button>
                    <button onClick={() => console.log("esqueci minha senha")} className="text-sm underline">Esqueci minha Senha</button>{/*não implementado devido à falta de tempo */}
                </div> 
            </form>
            <div className="flex justify-center mb-auto mx-auto p-4">
                <button type="button" onClick={() => ValidarEnvioLogin()}  className="font-lato-bold bg-logo-primary rounded-full w-56 h-16 text-white justify-center hover:cursor-pointer">ENTRAR</button>
            </div>
        </div>
   )
}